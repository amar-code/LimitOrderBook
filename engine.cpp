...
namespace CS {
    ...
    void Engine::HandleOrder(const Order &order) {
        ...
        if (oldOrderIt == hisOrderEntry_.end()) {
            ...
            if (order.action == MessageType::order) {
                // duplicate add
                ErrorMonitor::GetInstance().DuplicateAdd();

            } else if (order.action == MessageType ::cancel) {

                Remove(oldOrder, order);

            } else if (order.action == MessageType ::amend) {
    ...
    void Engine::Cancel( Order &order)
    {
	 CheckAskBidPx();

       Order oldOrder;
	auto &&OrderIt = hisOrderEntry_.find(order.orderId);
        if (OrderIt != hisOrderEntry_.end()) {
	    oldOrder=OrderIt->second;
            hisOrderEntry_.erase(OrderIt);
        }
	        if (oldOrder.status != OrderStatus::TradeDeleted) {
    ...
    void Engine::Amend(Order &order)
    {
	  CheckAskBidPx();
       Order oldOrder;
       int oldOrderSize=0, OrderDiff=0;
        auto &&OrderIt = hisOrderEntry_.find(order.orderId);
        if (OrderIt != hisOrderEntry_.end()) {
            oldOrder=OrderIt->second;
	    oldOrderSize=oldOrder.size;
		OrderDiff=oldOrderSize-order.size;
	    if( oldOrderSize >= order.size )
	    {
		OrderIt->second.size=order.size;
		    ...
		    hisOrderEntry_.insert(std::make_pair(order.orderId, oldOrder));
	    }
	  ...
	  PriceOrderBook &map = oldOrder.side == BUY ? buyOrderBook_ : sellOrderBook_;
			...
			oldOrder.ptr->size = order.size;
                        ordList.ChangeTradeSize(-OrderDiff);
        }
	}	
    ...
    void Engine::PrintOrderBook(std::ostream &os) const {
        ...
        for (auto it = sellOrderBook_.rbegin(); it != sellOrderBook_.rend(); ++it) {
            auto &&lst = it->second;
            for (auto &&sz : lst) {
                os << sz.orderId <<" sell " << sz.size <<" "<<it->first /100<<endl;
            }
        }
        ...
        for (auto it = buyOrderBook_.rbegin(); it != buyOrderBook_.rend(); ++it) {
            ...
            for (auto &&sz : lst) {
                os << sz.orderId << " buy " << sz.size<<" "<< it->first /100<<endl;;
            }
        }
    }
